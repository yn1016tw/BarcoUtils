...
**Unattended scheduling**: `src/chrome-remote-desktop/scheduled_run.bat.example` is a template (copy to `scheduled_run.bat`, gitignored — never commit the real file, it ends up holding a plaintext `--windows-password`) that runs `remote_login.py` against Barco Work Station (program path hardcoded to `python.exe C:\Project\BarcoUtils\src\timesheet\fill_timesheet2.py` — on the *remote* machine, a different path than this repo's local checkout) and puts the local machine back to sleep afterward (`rundll32.exe powrprof.dll,SetSuspendState 0,1,0`). Registered as the `BarcoWorkStation-MorningRun` Task Scheduler task: weekly trigger Mon–Fri 08:30, `WakeToRun` enabled, action = the local `scheduled_run.bat`. This relies on Task Scheduler's own wake-timer mechanism rather than any boot-reason detection — Windows can't reliably tell a scheduled RTC/WOL power-on from a manual one when resuming from a full shutdown (S5), but a *sleep* (S3), woken specifically to run a registered task, needs no such detection since the trigger itself is the only thing that fires it. Prerequisites applied directly on the host: `powercfg` `CONSOLELOCK` disabled on AC/DC (else the machine wakes to a lock screen that blocks all synthetic input — Windows' secure desktop rejects input injection from a regular user-mode process) and AC standby timeout set to 30 min (else it never sleeps in the first place, on this machine's original settings, and there's nothing for the task to wake *from*).

⚠️ **Credentials**: `scheduled_run.bat` (the real, gitignored file, not the `.example` template) currently passes `--windows-password` as a plaintext CLI argument — acceptable only because that specific file is excluded from git; the committed `.example` template must never contain a real password (`YOUR_PASSWORD_HERE` placeholder). This is a stopgap, not a real solution — deliberately left unresolved per this repo's owner pending real credential storage (e.g. the `keyring` package against Windows Credential Manager). Don't add plaintext credentials to any file that *does* enter git to work around it.
...
scripts/duvel_setup.bat     — Interactive Duvel device provisioning; options 1-6: mfg mode/SN/reboot/cert/SSID; [7] MDEP setup wizard; [8] run all 1-6; [9] run all 1-6 + auto setup wizard; [D] select device; device selection filters connected adb devices down to Duvel only (`getprop ro.barco.platform`==`w4duvel`, after ruling out Gen5 Button via `which g5configcli`) — non-Duvel devices are never listed or auto-selected
scripts/god_setup.bat       — God-mode production API device setup (separate from ADB-based duvel_setup.bat): mfg mode/SN/part number/MAC/WiFi/certs/OOBE via REST; [10] run all 1-9; also secure boot enable/read, FW version, part number read; device selection filters connected adb devices down to God only (`getprop ro.barco.platform`==`w4god`, after ruling out Gen5 Button via `which g5configcli`) — non-God devices are never listed or auto-selected
