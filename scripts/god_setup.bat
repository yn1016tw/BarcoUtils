...
echo   [J] Toggle Allow Update Over USB
...
if /i "%CHOICE%"=="J" goto TOGGLE_ALLOW_UPDATE_OVER_USB
...
:: ---- J. Toggle Allow Update Over USB ----
:TOGGLE_ALLOW_UPDATE_OVER_USB
echo.
echo [J] Toggle Allow Update Over USB on %DEVICE_SERIAL%...
echo ------------------------------------------------------------
set "USB_URI=content://com.barco.clickshare.configurationmanager.provider/clickshare/BaseUnit.Firmware.AllowUpdateOverUSB"
set "USB_CURRENT="
for /f "usebackq delims=" %%L in (`adb -s %DEVICE_SERIAL% shell content query --uri %USB_URI% 2^>nul`) do (
    set "USB_LINE=%%L"
    echo !USB_LINE! | findstr /C:"value=" >nul
    if not errorlevel 1 (
        set "USB_CURRENT=!USB_LINE:*value=!"
        if "!USB_CURRENT:~0,1!"=="=" set "USB_CURRENT=!USB_CURRENT:~1!"
    )
)
if defined USB_CURRENT (
    echo   Current value: %USB_CURRENT%
) else (
    echo   Current value: ^(not set^)
)
set /p "USB_VALUE=Enter new value (true/false): "
if /i not "%USB_VALUE%"=="true" if /i not "%USB_VALUE%"=="false" (
    echo Invalid value, must be true or false.
    pause
    goto MAIN_MENU
)
if defined USB_CURRENT (
    adb -s %DEVICE_SERIAL% shell content update --uri %USB_URI% --bind value:s:%USB_VALUE%
) else (
    adb -s %DEVICE_SERIAL% shell content insert --uri %USB_URI% --bind value:s:%USB_VALUE%
)
echo.
echo.
pause
