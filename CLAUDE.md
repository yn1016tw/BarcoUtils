...
Everything from "computer selected" to "ready to run the program" is one combined loop, `connect_and_login()`: each iteration re-reads the actual on-screen state and reacts, rather than assuming a fixed sequence of stages — some computers require a PIN and some don't; the connection may land straight on the Windows login screen (skipping the CRD toolbar's "Disconnect" button entirely, since that toolbar auto-hides over a fullscreen remote view) or, less often, straight on an already-unlocked desktop; and a screensaver on the remote end may sit in between, sometimes needing more than one wake attempt. Whenever none of PIN/login/"Disconnect" is recognized, it calls `win.wake()` (mouse jiggle + click below-center, avoiding the user-avatar tile that sits at vertical-center on the lock screen, + an arrow-key tap rather than a modifier key or a printable character, so it can't corrupt whatever gets typed into a field later) and polls again — a single click was observed to be unreliable against a real corporate slideshow screensaver, sometimes bringing up only the avatar/name with no password box yet, sometimes not dismissing it at all. PIN/Windows password are read via `getpass` if not passed as CLI args, never logged. The SPA at remotedesktop.google.com is often still blank right after navigation; one `F5` reload reliably renders the device list. Requires Tesseract-OCR installed separately (with `chi_tra`+`eng` language data) — `--tessdata-dir` (default `src/chrome-remote-desktop/tessdata/`, gitignored, not shipped in the repo — download `chi_tra.traineddata`/`eng.traineddata` from the tesseract-ocr/tessdata GitHub repo) and `--tesseract-cmd` (default `C:\Program Files\Tesseract-OCR\tesseract.exe`) can override the paths. `--pause-before {connect,pin,launch,disconnect}` pauses on `input()` before that step for manual inspection — only useful in an interactive terminal, since it raises `EOFError` immediately when stdin isn't attached (e.g. run from a background task); `pin` is the single pause point right before the combined connect/PIN/login loop starts.
