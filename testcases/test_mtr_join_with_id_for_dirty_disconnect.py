...
class MtrDirtyDisconnectTestRunner:
    ...
    def run_round(self, round_num: int, total_rounds: int) -> TestResult:
        ...
        try:
            ...
            self._logger.info("Rebooting Duvel to simulate dirty disconnect...")
            self._device.reboot()
            self._logger.info(f"Waiting for boot (timeout={_BOOT_TIMEOUT}s)...")
            self._device.wait_for_boot(timeout=_BOOT_TIMEOUT)
            r.reboot_complete = time.time()
            self._logger.info(f"Boot complete  (total: {r.total_seconds():.1f}s)")
            r.passed = True
...
def main():
    ...
    logger.info(f"MTR Dirty Disconnect Test  v{VERSION}")
